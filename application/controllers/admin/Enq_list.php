...
class Enq_list extends BaseController 
{     
	...
	{         
		...
		$this->load->model('Enq_list_model','company_leads');
	}
    ...
    public function index($param = '')
    {
        ...
        $this->load->view("backend/enq_list");
    ...
    public function ajax_list()
    {
        ...
        $list = $this->company_leads->get_datatables();		//echo '<pre>'; print_r($list); echo '</pre>'; exit;
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $company_leads) {
            $no++;
            $row = array();
			$row[] = '<input name="checkbox[]" class="checkbox1" type="checkbox" id="checkbox[]" value="'.$company_leads->id.'">';			$row[] = $company_leads->title;
			$row[] = $company_leads->name;
			$row[] = $company_leads->phone;
			$row[] = $company_leads->message;
			$row[] = $company_leads->email;
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->company_leads->count_all(),
                        "recordsFiltered" => $this->company_leads->count_filtered(),
                        "data" => $data,
                );
