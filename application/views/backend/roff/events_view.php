<div class="content-wrapper"> 
  ...
  <section class="content-header">
    <h1> <i class="fa fa-tachometer" aria-hidden="true"></i>Events<small>Control panel</small> </h1>
  </section>
...
function status_on(id,smethod)
{
    //alert(smethod);
	$.ajax({
        url : "<?php echo site_url('admin/events/ajax_status_on')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
{
    $.ajax({
        url : "<?php echo site_url('admin/events/ajax_status_off')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
...
function add_block()
{
    ...
    $.ajax({
        url : "<?php echo site_url('admin/events/ajax_add')?>",
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
...
function edit_block(id)
{
    ...
    //Ajax Load data from ajax
    $.ajax({
        url : "<?php echo site_url('admin/events/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			...
			$('[name="date_of_event"]').val(data.form_data.date_of_event);
			...
			{ 
				...
				if(data.form_data['img'+i])
				{
					...
					$('#img'+i+'-preview div').html('<img width="170px" height="170px" src="'+base_url+'evnts/'+data.form_data['img'+i]+'" class="img-responsive">'); // show photo
			...
			$.each(data.events, function(val,cat_data) 
			{
				$('#selector').append( $('<option> </option>').val(cat_data.id).html(cat_data.title) )
			});
			
			$("#selector").val(data.form_data.vehicle_type);
...
function save()
{
    ...
    if(save_method == 'add') {
        ...
        url = "<?php echo site_url('admin/events/ajax_update')?>";
    }
...
function delete_block(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('admin/events/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
$(document).ready(function() 
{
	...
	$("#del_all").on('click', function(e) 
	{
		if(confirm('Are you sure delete all data?'))
		{
			...
			$.ajax
			({
				url: '<?php echo site_url('admin/events/ajax_multi_delete')?>',
				type: 'post',
        ...
        <form action="#" id="form" class="form-horizontal">
          ...
          <div class="form-body">
            <div class="form-group">
              ...
              <div class="col-md-9">
                <input name="date_of_event" placeholder="date_of_event" class="form-control datepicker" type="text">
                <span class="help-block"></span> </div>
            </div>
          </div>
          <div class="form-group">
