<div class="content-wrapper"> 
  ...
  <section class="content-header">
    <h1> <i class="fa fa-tachometer" aria-hidden="true"></i> Projects <small>Control panel</small> </h1>
  </section>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('admin/projects/ajax_list')?>",
            "type": "POST"
        },
...
function status_on(id,smethod)
{
    //alert(smethod);
	$.ajax({
        url : "<?php echo site_url('admin/projects/ajax_status_on')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
{
    $.ajax({
        url : "<?php echo site_url('admin/projects/ajax_status_off')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
...
function add_block()
{
    ...
    $.ajax({
        url : "<?php echo site_url('admin/projects/ajax_addform')?>",
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			...
			$('#selector').append( $('<option> </option>').val('').html('Select Company Name') );
			$.each(data.company_arry, function(val,cat_data) 
			{
...
function order_block(id)
{
    save_method = 'order';
	 ...
	 //Ajax Load data from ajax
    $.ajax({
        url : "<?php echo site_url('admin/projects/add_order')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
...
function edit_block(id)
{
    ...
    //Ajax Load data from ajax
    $.ajax({
        url : "<?php echo site_url('admin/projects/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="title"]').val(data.form_data.title);
			 $('[name="lead_name"]').val(data.form_data.lead_name);
            ...
            if(data.form_data.photo)
            {
                ...
                $('#photo-preview div').html('<img width="170px" height="170px" src="'+base_url+'projects/'+data.form_data.photo+'" class="img-responsive">'); 
			...
			$("#selector").val(data.form_data.company_id);
        },
        error: function (jqXHR, textStatus, errorThrown)
        {
            alert('Error get data from ajax');
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('admin/projects/ajax_add')?>";
    } else {
        url = "<?php echo site_url('admin/projects/ajax_update')?>";
    }
...
function delete_block(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('admin/projects/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
$(document).ready(function() 
{
	...
	$("#del_all").on('click', function(e) 
	{
		if(confirm('Are you sure delete all data?'))
		{
			...
			$.ajax
			({
				url: '<?php echo site_url('admin/projects/ajax_multi_delete')?>',
				type: 'post',
      ...
      <div class="modal-body form">
        <form action="#" id="form" class="form-horizontal">
          ...
          <div class="form-group">
              ...
              <div class="col-md-9">
                <select id="selector" name="company_type" class="form-control">
            ...
            <div class="form-group">
              <label class="control-label col-md-3">Lead Name</label>
              <div class="col-md-9">
                <input name="lead_name" placeholder="Title" class="form-control" type="text">
                <span class="help-block"></span> </div>
            </div>        
            <div class="form-group" id="photo-preview">
              <label class="control-label col-md-3">Logo (100 * 100 px)(upto 50 kb)</label>
              <div class="col-md-9"> (No photo) <span class="help-block"></span> </div>
            </div>
            
            <div class="form-group">
              <label class="control-label col-md-3" id="label-photo">Upload Photo (100 * 100 px)(upto 50 kb)</label>
              <div class="col-md-9">
